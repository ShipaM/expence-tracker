...
В БД контейнера есть данные: несколько аккаунтов от ручных проверок и демо-аккаунт из сида.
...
npm run db:seed             # демо-аккаунт с данными для просмотра UI (см. ниже)
...
`packages/db/prisma/seed.ts` создаёт аккаунт **`demo@example.com` / `demo1234`** с девятью
категориями, ~270 транзакциями за четыре месяца и шестью регулярными платежами (среди них
просроченный и выключенный — чтобы было видно эти состояния UI).

- **Сид работает только со своим аккаунтом:** находит по email, удаляет вместе с данными
  (каскадом) и создаёт заново. Чужие записи в базе не трогает — там лежат реальные аккаунты.
  Не переделывайте его на `deleteMany()` по таблицам.
- **Данные детерминированы:** генератор случайных чисел с фиксированным сидом, повторный
  запуск даёт ту же базу.
- **Хэш пароля считается тем же `bcryptjs` с теми же 10 раундами, что в `AuthService`** —
  иначе вход демо-аккаунтом не сработает. Поэтому `bcryptjs` объявлен в devDependencies
  `@repo/db`, а не берётся хойстингом из `apps/api`.
- Запускается нативным TypeScript в Node (`node prisma/seed.ts`), без `tsx` и `ts-node`;
  импортирует собранный `../dist/index.js`, так что перед первым запуском нужен
  `npm run build --workspace @repo/db`. Импортировать `../src/client.ts` нельзя: внутри
  него путь `./generated/client.js` указывает на `.ts`-файл, а Node такие специфаеры не
  переписывает — резолвится только собранный `dist`.
- **`typecheck` сид не покрывает:** `tsconfig.json` включает только `src/**/*.ts`, а
  расширить `include` мешает `rootDir: "./src"` — с файлом вне корня падает `tsc --build`.
  Правя сид, проверяйте его запуском.
...
Vitest 4 во всех трёх воркспейсах, где есть тесты. Юнит-тесты лежат рядом с кодом, БД им не нужна:
`apps/api` — сервисы (`*.spec.ts`, окружение `node`), `packages/shared` — zod-схемы,
`apps/web` — компоненты (`*.spec.tsx`, окружение `jsdom` + Testing Library, см.
`apps/web/CLAUDE.md`). E2E — только у API, с реальной базой; конфиги, тестовая БД и секрет
описаны в `apps/api/CLAUDE.md`.

У каждого воркспейса свой `vitest.config.ts` с несовместимыми настройками трансформа
(`api` — swc из-за декораторов, `web` — плагин React из-за `jsx: "preserve"`), поэтому общего
корневого конфига нет и заводить его не нужно.
...
- Коммиты внутри ветки — по скиллу `.claude/skills/commit/SKILL.md` (см. [соглашение о
  коммитах](#соглашение-о-коммитах) ниже).
...
Правила коммитов вынесены в скилл **`.claude/skills/commit/SKILL.md`** — он описывает формат
сообщения (Conventional Commits, описание на английском одним предложением, без футеров и
упоминаний Claude), запрет на коммит в `master` и порядок проверок перед коммитом. Перед тем
как коммитить, читай его.
