...
2. **Страница, а не модальное окно.** Ссылка в меню уже ведёт на `/categories`, заодно чинится
   404. Модалка потребовала бы `Dialog` из shadcn, которого в проекте пока нет.
