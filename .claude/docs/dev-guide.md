...
npm run typecheck
npm run lint
npm run format:check        # или npm run format, чтобы сразу поправить
npm test
...
| один Prettier на весь монорепо (`.prettierrc.json` в корне)                   | конфиги в воркспейсах разойдутся и дадут шум в diff            |
